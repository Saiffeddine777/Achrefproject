...
export const fetchUserAPIThunk = createAsyncThunk("user/signin", async (credentials: { email: string, password: string }, thunkAPI) => {
    try {
        const res = await axios.post(`${EnvUrl}/api/users/signin`, credentials)
...
export const authenticatAPIThunk  = createAsyncThunk("user/authenticate" , async(credentials , thunkAPI)=>{
    try{
       const res = await axios.get(`${EnvUrl}/api/users/authenticate`,{withCredentials:true})
       return res.data
    }
    catch(error){
     thunkAPI.rejectWithValue(error)
...
export const signedInUserSlice = createSlice({
    ...
    extraReducers: ((builder) => {
        builder.
            ...
            builder.
            addCase(authenticatAPIThunk.pending, (state => {
                state.status = "loading"
            }))
            .addCase(authenticatAPIThunk.fulfilled, (state, action) => {
                state.status = "success"
                state.signedIn = action.payload
                state.error = null
            })
            .addCase(authenticatAPIThunk.rejected, (state, action) => {
                state.status = "failure"
                state.error = action.error.message || "an Error has Occured "
            })
